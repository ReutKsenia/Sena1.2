...
{
    public class NextDay3ViewModel : ViewModelBase
    {
        ...
        public NextDay3ViewModel()
        {
            timer.Interval = new TimeSpan(0, 0, 1);
            timer.IsEnabled = true;
            timer.Tick += (o, e) => {
                var d = new StringBuilder(date.AddDays(3).ToString("dddd", CultureInfo.GetCultureInfo("ru-ru")));
                d[0] = char.ToUpper(d[0]);
                DayWeek = d.ToString();
            };
            timer.Tick += (o, e) => { Day = date.AddDays(3).ToString("d MMMM", CultureInfo.GetCultureInfo("ru-ru")); };
            timer.Start();
        }
    }
}
